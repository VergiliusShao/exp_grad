...
class AOFtrl(object):
    ...
    def md(self,g):
        ...
        z = np.matmul(u*(alpha*_x)[..., None, :],v)+self.theta-(self.t+1.0)*g
        u,_z,v =np.linalg.svd(z,full_matrices=False,compute_uv=True)
        _y=_z/alpha
        x_val = np.maximum(alpha*_y-self.l1*self.beta,0.0)/(alpha+self.l2*self.beta)
...
def fmin(func, func_p,x0, upper,lower,l1=1.0,l2=1.0, maxfev=50,callback=None, epoch_size=10,eta=1.0 ):
    alg=AOFtrl(func=func,func_p=func_p,x0=x0,upper=upper,lower=lower,l1=l1,l2=l2,eta=eta)
    nit=maxfev
    fev=1
    y=None
    while fev <= maxfev:
