...
class AOExpFTRL(object):
    ...
    def md(self,g):
        ...
        _y=_z/alpha
        if self.l2 == 0.0:
            x_val = beta * np.exp(np.maximum(_y- self.l1*self.beta / alpha,0.0)) - beta
        else:
            a = beta
            b = self.l2*self.beta / alpha
            c = np.minimum(self.l1*self.beta / alpha - _y,0.0)
...
def fmin(func, func_p,x0, upper,lower,l1=1.0,l2=1.0, maxfev=50,callback=None, epoch_size=10,eta=1.0 ):
    alg=AOExpFTRL(func=func,func_p=func_p,x0=x0,upper=upper,lower=lower,l1=l1,l2=l2,eta=eta)
    nit=maxfev
    fev=1
    y=None
    while fev <= maxfev:
