...
from loss.cifer_resnet  import Grad_2p_Rad
...
def main(args):
    ...
    if spectral:
        ...
        get_l1=l1_norm
    if zo:
        print("use zeroth-order oracle")
    else:
        print("use first-order oracle")
    if zo and mini_batch:
        print("use mini batch")
    else:
        print("use single query")
    if device <0:
        ctx=mx.cpu()
    else:
        ctx=mx.gpu(device)
    ...
    if data=="CIFAR":
        ...
        num_classes=1
    if mini_batch:
        n=int(np.sqrt(num_iter))
    else:
        n=1
    delta=1.0/np.sqrt(num_iter*3*32*32)
    if not os.path.exists(result_path):
        os.makedirs(result_path)
    #for alg in [fista]:
    #for alg in [ao_exp_grad,ao_exp_ftrl,ao_grad,ao_ftrl]:
    if alg_name=="ao_exp_ftrl":
        alg=ao_exp_ftrl
    elif alg_name=="ao_grad":
        alg=ao_grad
    elif alg_name=="ao_ftrl":
        alg=ao_ftrl
    elif alg_name=="fista":
        alg=fista
    else:
        alg=ao_exp_grad
    file_path=data+'_'+mode+'_'+alg.__name__+'_'+str(zo)+'_'+str(mini_batch)
    ...
    for class_idx in range(0,num_classes):
        ...
        for sample_index in range(0,num_samples):
            ...
            while not found and current_index<max_index:            
                ...
                current_index+=1
                if data=="CIFAR" and (cem.label== class_idx and cem.correct):
                    found=True
                    break
            ...
            if mode=="PP":
                ...
                lower=cem.pn_lower
            if zo:
                if alg in [ao_exp_grad,ao_exp_ftrl]: 
                    func_p=Grad_2p_Rad(n=n,func=func,delta=delta)
                if alg in [ao_grad,ao_ftrl]: 
            ...
            alg.fmin(func=func, func_p=func_p, x0=init, lower=lower, upper=upper,l1=l1,l2=l2,maxfev=num_iter,callback=callback,epoch_size=1,eta=eta)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-t", "--num_iterations", type=int, default=2000)
    parser.add_argument("-s", "--spectral", type=bool, default=False)
    parser.add_argument("-d", "--device", type=int, default=0)
    parser.add_argument("-a", "--alg",choices=["ao_exp_grad","ao_exp_ftrl", "ao_grad","ao_ftrl","fista"], default="ao_exp_grad")
    parser.add_argument("-b", "--batch", type=bool, default=False)
    parser.add_argument("-g", "--gradient", type=bool, default=False)
    parser.add_argument("--l1", type=float, default=0.5)
    parser.add_argument("--l2", type=float, default=0.5)
    parser.add_argument("--mode", choices=["PN", "PP"], default="PP")
    parser.add_argument("--data", choices=["IMAGENET", "CIFAR"], default="CIFAR")
